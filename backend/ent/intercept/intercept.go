...
// The UserGroupEntitlementFunc type is an adapter to allow the use of ordinary function as a Querier.
type UserGroupEntitlementFunc func(context.Context, *ent.UserGroupEntitlementQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f UserGroupEntitlementFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.UserGroupEntitlementQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.UserGroupEntitlementQuery", q)
}

// The TraverseUserGroupEntitlement type is an adapter to allow the use of ordinary function as Traverser.
type TraverseUserGroupEntitlement func(context.Context, *ent.UserGroupEntitlementQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseUserGroupEntitlement) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseUserGroupEntitlement) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.UserGroupEntitlementQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.UserGroupEntitlementQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.UserGroupEntitlementQuery:
		return &query[*ent.UserGroupEntitlementQuery, predicate.UserGroupEntitlement, usergroupentitlement.OrderOption]{typ: ent.TypeUserGroupEntitlement, tq: q}, nil
